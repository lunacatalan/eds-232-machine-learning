...
knn_model <- nearest_neighbor(neighbors = tune()) %>% 
  ...
  # since we are using k-nearest neighbor (knn) there is only this engine
  set_engine("kknn") 
...
```{r, include=FALSE}
load(file = here::here("discussion", "knn_tune.rda"))
```

To see the accuracy of the different nearest neighbor numbers used. 
```{r}
collect_metrics()
```

To see what happens: For Classifications, use the ROC to determine which nearest number to use. The ROC curve is based on specificity and sensitivity; balances false negatives and false positives ; better idea of whats happening with class imbalances. 
```{r}
...
```{r}
show_best(knn_cv_tune, n = 1, 
          metric = "roc_auc") # can choose accuracy or area under the curve (auc)

knn_final <- finalize_workflow(knn_workflow, # give workflow
...
```{r}
# Predicts only the class values; the output will output a string 
...
# makes the class values into columns and calculates row probabilities 
# choose the max probabilities 
train_predict2 = predict(object = train_fit, new_data = train, #predict the training set
                         type = "prob") %>% # will spit out an individual prediction for each factor level within outcome variable
  bind_cols(train) #bind training set column to prediction
```
...
```{r}
accuracy(train_predict, truth = habitat, # original outcome variable
         estimate = .pred_class) #get training accuracy

# manually calculate the sensitivity and specificity
sensitivity(train_predict, truth = habitat, estimate = .pred_class) # measures false negatives; high .estimate means less false negatives
specificity(train_predict, truth = habitat, estimate = .pred_class) # measures false positives

test_roc_auc = roc_curve(train_predict2, # dataframe
                         habitat, `.pred_bathydemersal`, `.pred_bathypelagic`, `.pred_benthopelagic`, `.pred_demersal`, `.pred_pelagic`, `.pred_pelagic-neritic`, `.pred_pelagic-oceanic`, `.pred_reef-associated`)
```
