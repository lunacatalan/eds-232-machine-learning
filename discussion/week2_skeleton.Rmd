...
fish_clean = read_csv(here("discussion", "fish_clean.csv")) %>% 
  filter(habitat == "demersal" | habitat == "reef-associated") %>% # choose either of these habitats
  mutate(habitat = case_when( # set binary variables
    habitat == "demersal" ~ 0,
...
```{r}
fish_split <- fish_clean %>% 
  initial_split(prop = 0.8, # set the data proportion
                strata = habitat) # get representative sample 
...
```{r}
 ...
 #get training data
train <- training(fish_split)

 #get testing data
test <- testing(fish_split)
...
```{r}
fish_recipe <- 
  recipe(habitat ~ trophic_level_impute + length_impute + shallow_impute + deep_impute + vulnerability + shape_impute + food_impute, # equation is outcome ~ predictor variable
         data = train) %>% 
...
```{r}
# when using recipe in a workflow, dont have to specify prep or bake

fish_wf_lm <- workflow() %>% # create workflow 
  add_recipe(fish_recipe) %>% # add model
  add_model(lm_model) # add recipe
```
...
```{r}
train_predict <- predict(object = fish_wf_fit,
                         new_data = train) %>% 
  bind_cols(train)
...
test_predict <- predict(object = fish_wf_fit,
                         new_data = test) %>% 
  bind_cols(test) %>% 
...
head(test_predict)
```
Lower values for the RMSE are better... but it depends on what the units are.
