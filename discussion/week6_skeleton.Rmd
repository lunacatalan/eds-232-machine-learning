...
```{r, include=FALSE}
load(file = here::here("discussion",  "rf_tune.rda"))
```
...
```{r}
# hot much infor it learns through each iteration of the tree
...
```{r, include=FALSE}
load(file = here::here("discussion","xgb_tune.rda"))
```
...
autoplot(xgb_cv_tune) + #plot cv results for parameter tuning
  theme_bw()
```
Learning rate is more important than the number of trees. New tree is new step, and if you have an optimized learning rate, you can decide how many steps you need to take to reach that optimized learning rate. 
...
test_predict2_xbg = predict(train_fit_xgb, test, type = "prob") %>%  #get prediction probabilities for test 
  bind_cols(test) %>%  #bind to testing column
  mutate(habitat = as.factor(habitat))
```
