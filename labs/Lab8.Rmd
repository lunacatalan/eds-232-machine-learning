...
covtype <- read_csv(here::here("labs", "data", "covtype_sample.csv")) %>% 
  ...
  mutate(across(11:53, as.factor)) # convert binary to initial
...
cov_recipe <- recipe(cover_type ~ ., data = cov_train) %>% 
  step_dummy(all_nominal_predictors(), # categorical predictors
             one_hot = TRUE) %>% 
  step_normalize(all_numeric_predictors()) # all the numeric variables
...
rf_tune <- rf_workflow %>% 
  tune_grid(
    resamples = vfolds, # add folds
    grid = 10 # number of combos of mtry and trees
  )
