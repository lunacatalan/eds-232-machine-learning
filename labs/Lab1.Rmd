...
When is the best time to buy pumpkins? - date / price
...
new_pumpkins <- pumpkins %>% 
  filter(str_detect(package, "bushel")) # select only rows with 'bushel' in them 
...
This plot is not very useful except to identify general trends. It seems as though there are two price points, though it is hard to identify if those belong to different groups. Also, whereas the fist plot had data from day 0 to 365, this plot only contains data from day 225 to 350. This is about the time that the pumpkin holidays are happening, so it makes sense.
...
There are 10 total cities that are represented in the data set. Each of the cities has been assigned a value from 0 to 9.
...
print(cor(baked_pumpkins$city_name, baked_pumpkins$price))
...
The variety is most highly correlated with price. This might be because variety has to do with the size of the pumpkin, and it makes sense that the sizes of the pumpkin will affect the price.
...
metrics(data = lm_results,
        truth = price, # variable that is being predicted
        estimate = .pred)
```
...
plot_results <- lm_results %>% # use bound predictions / test set 
   bind_cols(package_encode %>%
             rename(package_integer = package)) %>% # add columns from package_encode df and rename col
   relocate(package_integer, .after = package) # reorder columns 
...
Fitting a line to this data is not great because the data is categorical, and this model is underfitting the data. There are a lot of points that are not represented by the model, which means that using this model would not lead to very accurate predictions for prices. 
